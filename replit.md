...
8. **Customer Messaging with Staff Replies** – Two-way communication: customers send messages (subject + body) via "Message Us" button in their dashboard; staff view in Inbox and reply directly per message. Customers see staff replies in a "My Messages" section on their dashboard, with a badge indicator on the Message Us button when replies are received. `messages` table: id, customerId, customerName, subject, message, isRead, createdAt, staffReply, repliedAt, repliedByName. Routes: `POST /api/customer/messages` (send), `GET /api/customer/messages` (customer view), `POST /api/messages/:id/reply` (staff reply), `GET /api/messages` (staff view all).
